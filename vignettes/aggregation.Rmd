...
```
    time_from time_to region_from region_to Nnet
1:         0      10           1         1   19
2:         0      10           1         1   19
3:         0      10           1         1   19
4:         0      10           1         1   19
...
For each pair(time_from-time_to, region_from-region_to) there are n random values in the last column. One can use again the mean, mode or median to have an estimate of the number of individuals moving from one region to another at a certain time.

The following code shows how to compute the mean number of individuals movinf from region 1 to all regions
...
regions <- as.numeric(unique(nnetOD$region_from))


for(r in regions) {
    print(paste0("From region ", reg_from, " to region ", r, " number of individuals ", round(mean(nnetOD[time_from==t_from][time_to==t_to][region_from==reg_from][region_to==r]$Nnet))))
}    
```

Thenext example shows how to build the entire origin-destination matrix for a certime time interval:
```{r}
t_from <- 550
t_to <- 560
...
for(r1 in regions_from) {
    for(r2 in regions_to) {
        ODmat[r1,r2] <- round(mean(nnetOD[time_from==t_from][time_to==t_to][cell_from==r1][cell_to==r2]$Nnet))
    }
}    
